...
Semantic HTML makes it clear to the browser the specific meaning and content of the page. It also helps the next developer quickly see the meaning of each section.

2. Name two big differences between ```display: block;``` and ```display: inline;```.
display:block - displays the element as a block
display;inline - displays the element on the same line inside the current block.
...
git allows each team member to contribute code to the same project. Allowing each member to have their own branch and version control.
